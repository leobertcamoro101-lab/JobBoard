...
const PostJobPage = () => {
  ...
  const inputClass = (hasError?: boolean) =>
    `w-full bg-gray-800 border ${hasError ? 'border-red-500/50' : 'border-gray-700'}
     text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-violet-400
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="space-y-5">
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-2xl p-6 space-y-4">
          <h2 className="text-white font-bold">Salary (Optional)</h2>
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-2xl p-6 space-y-4">
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">
              Requirements <span className="text-gray-600">(one per line)</span>
            </label>
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-2xl p-6">
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Application Email *</label>
            <input {...register('apply_email')} type="email" placeholder="jobs@yourcompany.com"
              className={inputClass(!!errors.apply_email)} />
            {errors.apply_email && <p className="text-red-400 text-xs mt-1">{errors.apply_email.message}</p>}
          </div>
        </div>
